/*------------------------------------------------------------------------------------------
  ...
  PURPOSE      : Build Candy Sales SASGF14 cube for SGF13 
|-----------------------------------------------------------------------------------------*/
LIBNAME pgdev POSTGRES  PRESERVE_COL_NAMES=YES  PRESERVE_TAB_NAMES=YES  READBUFF=1000  INSERTBUFF=1000  DATABASE=dev  SERVER=defiant  SCHEMA=public  USER=pegasus  PASSWORD="{SAS002}1D5793391C1104E20E3CF4CD2A793E2B" ;

%let syscc=0;

proc olap
   CUBE                   = "/Projects/SGF2014/Cubes/Candy Sales SASGF14"
   DELETE;
...
proc olap
   CUBE                   = "/Projects/SGF2014/Cubes/Candy Sales SASGF14"
   PATH                   = '/projects/SGF2014/cubes'
   DESCRIPTION            = 'Candy Sales demo cube for SGF2014'
...
proc olap;
   ...
   /* Rolling month sets */
   DEFINE SET '[Candy Sales SASGF14].[Rolling 12 Months]' as 'tail([Time].[YMD].[Month].members,12)';
   DEFINE SET '[Candy Sales SASGF14].[Rolling 24 Months]' as 'tail([Time].[YMD].[Month].members,24)';
   DEFINE SET '[Candy Sales SASGF14].[Rolling 36 Months]' as 'tail([Time].[YMD].[Month].members,36)';
   DEFINE SET '[Candy Sales SASGF14].[Rolling 7 Days]' as 'tail({[Time].[YMD].[Date].members},7)';

   /* Rolling time aggregate members */
   DEFINE MEMBER '[Candy Sales SASGF14].[Time].[YMD].[All Years].[Rolling 12 Months]' as 'aggregate( tail([Time].[YMD].[Month].members ,12) )';
   DEFINE MEMBER '[Candy Sales SASGF14].[Time].[YMD].[All Years].[Rolling 24 Months]' as 'aggregate( tail([Time].[YMD].[Month].members ,24) )';
   DEFINE MEMBER '[Candy Sales SASGF14].[Time].[YMD].[All Years].[Rolling 36 Months]' as 'aggregate( tail([Time].[YMD].[Month].members ,36) )';
   
   /* Ratio */
   DEFINE MEMBER '[Candy Sales SASGF14].[Measures].[Sales Price Per Unit]' AS
    '([Measures].[Sales Amount] / [Measures].[Units Sold]), format_string="dollar20.0"';

   /* Rolling Average */
   DEFINE MEMBER '[Candy Sales SASGF14].[Measures].[Sales 3M Rolling]' AS
        'Avg(LastPeriods(3,[Time].[MonthYear].CurrentMember),[Measures].[Sales Amount])';
   DEFINE MEMBER '[Candy Sales SASGF14].[Measures].[Sales 6M Rolling]' AS
        'Avg(LastPeriods(6,[Time].[MonthYear].CurrentMember),[Measures].[Sales Amount])';

   /* Percent Change - Sales Growth */
   DEFINE MEMBER '[Candy Sales SASGF14].[Measures].[Sales Growth]' AS
   ...
   DEFINE MEMBER '[Candy Sales SASGF14].[Measures].[Team AVG Sales]' AS
    '([Customer].[Org Structure].CurrentMember.Parent,[Measures].[Avg Sales]), SOLVE_ORDER=0, format_string="dollar20.0"';

   DEFINE MEMBER '[Candy Sales SASGF14].[Measures].[Team AVG 3M Sales]' AS
   ...
   DEFINE MEMBER '[Candy Sales SASGF14].[Measures].[Diff Team AVG Sales]' AS
    '([Measures].[Avg Sales] - [Measures].[Team AVG Sales]), SOLVE_ORDER=1, format_string="dollar20.0"';

   DEFINE MEMBER '[Candy Sales SASGF14].[Measures].[Diff Team AVG 3M Sales]' AS
