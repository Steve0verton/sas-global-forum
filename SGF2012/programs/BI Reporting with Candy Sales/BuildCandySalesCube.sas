...
PROC OLAP
   CUBE                   = "/Projects/SGF2012/Cubes/Candy Sales"
   DELETE;
...
PROC OLAP
   CUBE                   = "/Projects/SGF2012/Cubes/Candy Sales"
   PATH                   = '/projects/SGF2012/cubes'
   DESCRIPTION            = 'Candy sales demo cube for SGF2012'
   FACT                   = SGF2012.FACT_CANDY_SALES
;
   ...
   /* Rolling sets */
   DEFINE SET "[Candy Sales].[Rolling 12 Months]" as "Tail([Time].[MonthYear].[Month Year].Members ,12)";
   DEFINE SET "[Candy Sales].[Rolling 24 Months]" as "Tail([Time].[MonthYear].[Month Year].Members ,24)";
   DEFINE SET "[Candy Sales].[Rolling 36 Months]" as "Tail([Time].[MonthYear].[Month Year].Members ,36)";
   DEFINE SET "[Candy Sales].[Rolling 7 Days]" as "tail({ [Time].[YMD].[Date].AllMembers},7)";
   

   /* Rolling time aggregate members */
   DEFINE MEMBER "[Candy Sales].[Time].[MonthYear].[All Years].[Rolling 12 Months]" as 'Aggregate( Tail([Time].[MonthYear].[Month Year].Members ,12) )';
   DEFINE MEMBER "[Candy Sales].[Time].[MonthYear].[All Years].[Rolling 24 Months]" as 'Aggregate( Tail([Time].[MonthYear].[Month Year].Members ,24) )';
   DEFINE MEMBER "[Candy Sales].[Time].[MonthYear].[All Years].[Rolling 36 Months]" as 'Aggregate( Tail([Time].[MonthYear].[Month Year].Members ,36) )';

   DEFINE SET '[Candy Sales].[Rolling 12 Months Plus Summary]' as "
      {
         [Rolling 12 Months],
         [Time].[MonthYear].[All Years].[Rolling 12 Months],
         [Time].[MonthYear].[All Years].[Rolling 24 Months],
         [Time].[MonthYear].[All Years].[Rolling 36 Months]
      }";
