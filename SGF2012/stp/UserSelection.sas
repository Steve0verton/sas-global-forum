/****************************************************************************/
/* Code originally written by SAS Institute and provided with               */
/* SAS Foundation.  Code is enhanced for demonstration purposes by          */
...
/*** Create session with a 5 min session timeout **/
...
data &extractlibref..ldapusers                                                               
     ...
     filter="(&(&(displayName>=A)(displayName<=C))  (distinguishedName=*)  )";
     %ldapextrpersons
     
     filter="(&(&(displayName>=C)(displayName<=D))  (distinguishedName=*) )";
     %ldapextrpersons
     
     filter="(&(&(displayName>=D)(displayName<=E))  (distinguishedName=*)  )";
     %ldapextrpersons
     ...
     filter="(&(&(displayName>=I)(displayName<=K))  (distinguishedName=*)  )";
     %ldapextrpersons
     
     filter="(&(&(displayName>=K)(displayName<=M))  (distinguishedName=*) )";
     %ldapextrpersons
     
     filter="(&(&(displayName>=M)(displayName<=O))  (distinguishedName=*)  )";
     %ldapextrpersons
    
     filter="(&(&(displayName>=O)(displayName<=Q))  (distinguishedName=*)  )";
     %ldapextrpersons
     
     filter="(&(&(displayName>=Q)(displayName<=S))  (distinguishedName=*)  )";
     %ldapextrpersons
     
     filter="(&(&(displayName>=S)(displayName<=T))  (distinguishedName=*)  )";
     %ldapextrpersons
     
     filter="(&(&(displayName>=T)(displayName<=U))  (distinguishedName=*)  )";
     %ldapextrpersons
     
     filter="(&(&(displayName>=U)(displayName<=W))  (distinguishedName=*)  )";
     %ldapextrpersons
     
     filter="(&(&(displayName>=W)(displayName<=Y))  (distinguishedName=*)  )";
     %ldapextrpersons
     
     filter="(& (displayName>=Y)  (distinguishedName=*) )";
     %ldapextrpersons          
...
/** Load macro variables for input form **/
proc sql;
  /** Get count of users **/
...
/** Write HTML using macro and users from Active Directory **/
  %macro person_list;
...
/** Custom input form to select users **/
data _null_;
	file _webout;
	put '<html>';
	put '<form name="Users" method="post" action=' "&_THISSESSION" '>';
	put '<input type="hidden" id="_program" name="_program" value="/Shared Data/SGF2012/Stored Processes/Active Directory User Import 2" />';
...
/** copy necessary macro variables to SAVE library for use in next stored process **/
